...
/*   Updated: 2022/08/19 12:21:11 by avillar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exec_bypath(t_llist *list, t_pipe *pip, char **arg_tab)
{
	...
	//free (pip->cmd);
	if (list->path)
		ft_cmdnotf("command not found: ", pip->cmd);
	else
		ft_cmdnotf2(pip->cmd, " : No such file or directory\n");
